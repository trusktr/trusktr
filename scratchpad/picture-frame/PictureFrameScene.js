...
export const PictureFrameScene = element('picture-frame-scene')(
	class PictureFrameScene extends Element {
		...
		connectedCallback() {
			...
			// Hide the orb visual
			const orb = this.root.querySelector('flickering-orb')
			orb.root.querySelector('lume-sphere').remove()

			const glass = this.root.querySelector('#glass')
			setTimeout(() => {
				console.log(glass.three.material.transmission)
				glass.three.material.transmission = 1
				setTimeout(() => {
					console.log(glass.three.material.transmission)
				}, 2000)
			}, 2000)
